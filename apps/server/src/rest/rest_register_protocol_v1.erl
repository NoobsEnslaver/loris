...
handle(<<"POST">>, Req, #q_state{tmp_state = #{'session' := Session}, req_body = #{<<"login">> := Login
                                                                                  ,<<"password_hash">> := Pwd
                                                                                  ,<<"name">> := Name
                                                                                  ,<<"group">> := BGroup
                                                                                  ,<<"access_level">> := AL1}} = State, _Args) when BGroup =:= <<"users">> orelse BGroup =:= <<"guests">> orelse BGroup =:= <<"administrators">> ->
    ...
    {Req, NewState, []};
handle(<<"POST">>, _Req, State, _Other) ->
    {_Req, State#q_state{code = 400}, _Other};        % bad request
