...
| c2s_call_invite               | 34            | msisdn, sdp_offer                     | нет или s2c_call_bye               | Приглашение начать звонок с пользователем     |
...
| s2c_call_invite               | 119           | msisdn, sdp_offer                                     | Вам звонят                                    |
...
Управление звонками происходит по вебсокету через ws_chat_protocol. В вашем распоряжении 4 команды:

* c2s_call_invite - Позвонить человеку, либо принять его звонок.
...
1. Отправляем c2s_call_invite с указанием msisdn абонента и sdp_offer - набор параметров, сообщающих оппоненту информацию, необходимую для установления связи с Вами, такую как поддерживаемые кодеки и тд.
2. Если такого абонента не существует, либо он занят, либо не в сети, Вы получаете c2s_call_bye с указанием причины завершения звонка.
3. Если пользователь в сети и у него открыт вебсокет, начинается слежение за состоянием его процесса, если связь с абонентом оборвётся, вы тут же получите сообщение c2s_call_bye с кодом 500.
4. Оппонент получает сообщение s2c_call_invite с теми же полями и, если его всё устраивает, ему рекомендуется отослать c2s_call_ack, чтобы звонящий знал, что ожидается действие пользователя, а не происходит доставка сообщения о приглашении на звонок и мог запустить "гудки ожидания".
5. Когда клиент готов принять звонок, он отправляет c2s_call_invite с номером и sdp_offer.
6. Теперь, когда клиенты обменялись sdp_offer, начинается установка p2p соединения, для этого каждый из клиентов подыскивает ice кандидатов и предлагает их оппоненту с помощью сообщения s2c_call_ice_candidate.
7. Устанавливается свясь и все рады, сервер уже не отслеживает эту часть т.к. соединение p2p.
8. Звонок заканчивается, когда одна из сторон шлёт сообщение c2s_call_bye (код 200 - нормальное завершение звонка), другая сторона получает это сообщение от сервера и обновляет статусы пользователей, выводя их из состояние "заняты".

TODO:
* Введение TURN серверов, их список будет получаться от сервера
