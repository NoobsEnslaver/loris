...
Передача файлов в чате производится через загрузку файла на сервер и отправки получателю id файла, который он потом заберёт с сервера.
Для загрузки файла на сервер отправляем POST с content-type=multipart/form-data (так же, как отправляет файлы обычная html форма) на адрес

>       localhost:8080/v1/session/$SESSION_ID/file

в случае успешной загрузки получаем код ответа 201 и id файла в теле ответа. Затем отправляем id файла через чат системным сообщением, например @file $FILE_ID, пользователь, получивший id файла, может получить его с сервера, сделав GET запрос на адрес

>       localhost:8080/v1/session/$SESSION_ID/file/$FILE_ID

Коды ответа:

* 201: файл успешно загружен на сервер
* 411: не указана длина файла
* 413: файл слишком велик (текущее ограничение на размер 16 мб)
* 403: нет прав на доступ к файлу
